using Meshes, Tables, Distances, Statistics, LinearAlgebra, CategoricalArrays
using CircularArrays, StaticArrays, SparseArrays, PlyIO, Unitful, Rotations
using Test, Random, ReferenceTests, ImageIO
...
clipped = clip(poly, other, Meshes.GreinerHormann())
clippedRes = Ring([P2(5, 3), P2(4, 4), P2(2, 4), P2(0, 2), P2(5, 2)])

@test issimple(clipped)
# @show vertices(clipped) .≈ vertices(clippedRes)
# vertices(clipped) .- vertices(clippedRes) .|> norm .|> println;
all([isapprox(c1, cr1, atol = 5e-3) for (c1, cr1) in zip(vertices(clipped), vertices(clippedRes))])
